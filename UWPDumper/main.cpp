...
std::uint32_t __stdcall DumperThread(void* DLLHandle)
{
	...
	IPC::PushMessage(L"\033[92m\nUWPDumper Build date(%ls : %ls)\n", WIDEIFY(__DATE__), WIDEIFY(__TIME__));
	IPC::PushMessage(L"\033[96m\t\033(0m\033(Bhttps://github.com/SEWsam/UWPDumper-lib\n");
	IPC::PushMessage(L"\033[33m\t\033(0m\033(Bforked from https://github.com/Wunkolo/UWPDumper\033[0m\n");
	...
	std::size_t i = 0;
	bool failed = false;
	for( const auto& File : FileList )  // all errors below should have matching enums and be passed as those instead of strings. (make a second queue-- pass structs)
	{	
		++i;
		const float Progress = i / float(FileList.size()) * 100;
